...
class AudioPlayer {
    ...
    static func soundLength(_ sound: Sound, speed: Double) -> Double {
        guard let soundUrl = sound.url else { return 0 }
        do {
    ...
    func playSound(_ sound: Sound, node audioPlayerNode: AVAudioPlayerNode = AVAudioPlayerNode(), pitch: Double, speed: Double, volume: Float, filter: AVAudioUnitDistortionPreset?, length: Double) {
    ...
    private func createBuffer(sound: Sound) -> AVAudioPCMBuffer? {
        var res: AVAudioPCMBuffer?
        guard let url = sound.url else { return res }
        do {
